/* /_ui/js/trans-path/lib/jquery-1.7.2.min.js */
...
/* /_ui/js/trans-path/lib/jquery-ui-1.10.3.custom.min.js */
...
/* /_ui/js/trans-path/lib/jquery.sidr.min.js */
...
/* /_ui/js/trans-path/lib/fastclick.js */
/**
...
/* /_ui/js/trans-path/lib/jquery.hoverIntent.minified.js */
/*!
...
/* /_ui/js/trans-path/classes/meganav.js */
/**
 * @author POP JS-ID [CN]
 * @version: 0.5.
 * @description: fly-out mega-nav
 * @dependencies: jQuery 1.6+
 */
/*** start MegaNav ***/
POP.MegaNav = function (options) {
	var self = this;

	// defaults
	this.options = $.extend({
        elNavContainer: '#nav-main',
		elNavItem: 'a.nav-item',
		elNavSub: 'div.nav-sub',
        elNavSubColumn: 'div.column',
        elLastNavLink: 'a:last',
        elNonNavLink: '#content a'
	}, options || {});

	this.elNavContainer = $(this.options.elNavContainer);
	this.elNavItem = this.elNavContainer.find(this.options.elNavItem);
	this.elNavSub = this.elNavContainer.find(this.options.elNavSub);
	this.elNavSubColumn = this.elNavContainer.find(this.options.elNavSubColumn);
	this.elLastNavLink = this.elNavContainer.find(this.options.elLastNavLink);
	this.elNonNavLink = $(this.options.elNonNavLink);
	
	this._lenNavItems = this.elNavItem.length;

    this.columnCount();
    this.elNavSub.hide().removeClass('invisible');

	this.bindEvents();

};

POP.MegaNav.prototype = {
	bindEvents: function () {
		var self = this;
		...
		this.elNavItem.bind({
			touchend: function(event) {
				...
				var eTarget = $(event.target),
					subNav = eTarget.parent().find('.nav-sub');
					
				if( subNav.css('display') === 'none' ) {
					$('a.over').removeClass('over');
					self.__navMouseOver(event);
				} else {
					self.__navMouseOut(event);
				}
			},
		...
		this.elNavSub.bind({
			mouseenter: function(event) {
				self.__navSubMouseOver(event);
			},
			mouseleave: function(event) {
				self.__navSubMouseOut(event);
			},
			blur: function(event) {
				self.__navMouseOut(event);
			},
    ...
    columnCount: function () {
		var self = this,
			lenElNavSub = this.elNavSub.length,
			elNavSub,
			navSubColumns;

        for (var i = 0; i < lenElNavSub; i++) {
            elNavSub = $(this.elNavSub[i]);
            if (elNavSub) {
                navSubColumns = elNavSub.children();
                if (navSubColumns.length > 4) {
                    elNavSub.addClass('nav-sub-large');
                }
            }
        }
    },
    showDropDown: function (elTarget) {
		var self = this;
			elTarget = $(elTarget),
			elNavSub = elTarget.siblings(this.options.elNavSub);
		elTarget.addClass('over');
    ...
    hideDropDown: function (elTarget) {
		...
		elTarget.removeClass('over');

		clearTimeout(this.hoverDelay);
		this.fadeDelay = setTimeout(function () {
			elNavSub.fadeOut(200);
		}, 100);
    },
    __navMouseOver: function (e) {
		var self = this,
			elTarget = e.currentTarget;
		this.elNavSub.hide();
    ...
    __navSubMouseOver: function (e) {
		var self = this,
			elTarget = $(e.currentTarget).siblings(this.options.elNavItem);
		this.showDropDown(elTarget);
    },
...
var navSearch = {
    ...
    initialize: function () {
        ...
        searchButton.click(function(event) {
            event.preventDefault();
            if(searchButton.hasClass('search-button-over')){
    ...
    hideSearch: function () {
        jQuery('#search-events').hide();
        jQuery('body').removeClass('search-opened');
        if(jQuery('.mini-cart').length){
            jQuery('.mini-cart').css('top','56px');
        }
        jQuery('.search-button').removeClass('search-button-over');
    },
    showSearch: function (){
        ...
        if(jQuery('.mini-cart').length){
            jQuery('.mini-cart').css('top','109px');
        }
        jQuery('#search-events').show();
        jQuery('.search-button').addClass('search-button-over');
        jQuery('.search-container').find('input[type="text"]').focus();

        if(!navSearch.activated){
            navSearch.activated = true;
            jQuery(document).mouseup(function (e){
            var clickedContainer = jQuery(e.target);
            if (!searchContainer.is(clickedContainer) && searchContainer.has(clickedContainer).length === 0){
                   navSearch.hideSearch();
                }
...
var navCart = {
    initialize: function () {
        if(jQuery('.mini-cart-close').length){
            jQuery('body').addClass('cart-opened');

            jQuery('.mini-cart-close').click(function(event) {
                event.preventDefault();
                navCart.hideCart();
                jQuery('body').removeClass('cart-opened');
            });

            jQuery('.mini-cart-button').click(function(event) {
                event.preventDefault();
                navCart.showCart();
                jQuery('body').addClass('cart-opened');
            });
    ...
    hideCart: function () {
        ...
        jQuery('.mini-cart-button').show();
    },
    showCart: function (){
        jQuery('.mini-cart').show();
        jQuery('.mini-cart-button').hide();
    }
}

$(function () {
	SOH.meganav = new POP.MegaNav();
    navSearch.initialize();
    navCart.initialize();
    
    if(jQuery('.nav-utility-state .nav-utility-actions').length){
        jQuery('.nav-utility-state > li').hoverIntent(
          function () {
            jQuery(this).addClass('active');
            jQuery(this).find('.nav-utility-actions').show();
          }, 
          function () {
            jQuery(this).removeClass('active');
            jQuery(this).find('.nav-utility-actions').hide();    
          }
        ); 
    }   
});

/* /_ui/js/trans-path/classes/modal-base.js */
/**
 * @author POP JS-ID [CN]
 * @version: 0.6.
 * @description: base class for creating modal windows
 * @dependencies: jQuery 1.6+
 */
/*** start ModalBase ***/
POP.UI.ModalBase = function (popoverID, options) {
	var self = this;

	// element references
	this.window = POP.CONFIG.window;
	this.body = POP.CONFIG.body;
	this.modalOverlay = POP.UI.modalOverlay;
	...
	this.options = $.extend({
		...
		closeBtnInnerHTML: '<span class="offscreen">Close(X)</span>',
		activeClass: 'active',
		leftOffset: 0,
		topOffset: 0,
		minTopSpacing: 10,
		...
		customEventPrfx: 'POP:UI:ModalWindow'
	}, options || {});
	this.isActivated = false;

};

POP.UI.ModalBase.prototype = {
	bindEvents: function () {
		var self = this;

		if (this.options.includeCloseBtn) {
			this.btnCloseX.bind('click', function (event) {
				if (self.isActivated) {
					event.preventDefault();
					event.stopPropagation();
					self.closePopover();
		...
		if (this.options.backgroundClickClose) {
			if (this.options.useModalOverlay) {
				this.modalOverlay.bind('click', function (event) {
					if (self.isActivated) {
						self.closePopover();
					}
				});
	...
	createWidgetMarkup: function () {
		...
		this.popoverHTML = '<div id="' + this.popoverID + '" class="' + this.options.popoverClass + '" role="alertdialog" tabindex="-1" aria-expanded="false" aria-hidden="true">';
		if (this.options.addHeaderAndFooter) {
			this.popoverHTML += '	<div class="' + this.options.popoverClass + '-header"></div>'
							 +  '	<div class="' + this.options.popoverClass + '-content"></div>'
							 +  '	<div class="' + this.options.popoverClass + '-footer"></div>';
		} else {
			this.popoverHTML += '	<div class="' + this.options.popoverClass + '-content"></div>';
		}
		this.popoverHTML += '</div>';
		this.elPopover = $(this.popoverHTML);
		this.btnCloseX = $('<a class="' + this.options.closeBtnClass + '" href="#close" title="close window">' + this.options.closeBtnInnerHTML + '</a>');
		if (this.options.includeCloseBtn) {
			this.elPopover.append(this.btnCloseX);
		}
		this.modalOverlay.after(this.elPopover);
		this.elPopover.hide();
	...
	setPosition: function () {
		...
		this.popoverWidth = this.elPopover.outerWidth();
		this.popoverHeight = this.elPopover.outerHeight();
		this.windowWidth = this.window.width();
		this.windowHeight = this.window.height();

		// assumes position:fixed by defalt. if position:absolute, the top pos gets trickier.
		this._isPosAbs = (this.elPopover.css('position') == 'absolute') ? true : false;
		this.leftPos = (((this.windowWidth - this.popoverWidth) / 2) + this.options.leftOffset);
		this.topPos = (((this.windowHeight - this.popoverHeight) / 2) + this.options.topOffset);
		if (this.topPos < this.options.minTopSpacing) {
			this.topPos = this.options.minTopSpacing;
		}
		this.elPopover.css({left: this.leftPos + 'px', top: this.topPos + 'px'});
	...
	_openPopover: function () {
		...
		if (this._isPosAbs) {
			adjTopPos = this.window.scrollTop() + this.options.minTopSpacing;
			this.elPopover.css({top: adjTopPos + 'px'});
		}
		this.elPopover.fadeIn(this.options.fadeInOutSpeed, 'linear', function () {
			self.elPopover.addClass(self.options.activeClass).attr({'aria-expanded': 'true', 'aria-hidden': 'false'});
			self.btnCloseX.focus();
			$.event.trigger(self.options.customEventPrfx + ':popoverOpened', [self.popoverID]);
		});
	...
	openPopover: function () {
		...
		if (this.options.useModalOverlay) {
			this.modalOverlay.fadeIn(this.options.fadeInOutSpeed, 'linear', function () {
				self._openPopover();
			});
		} else {
			this._openPopover();
	...
	closePopover: function () {
		...
		this.elPopover.fadeOut(this.options.fadeInOutSpeed, 'linear', function () {
			self.elPopover.removeClass(self.options.activeClass).attr({'aria-expanded': 'false', 'aria-hidden': 'true'});
			if (self.options.useModalOverlay) {
				self.modalOverlay.fadeOut(self.options.fadeInOutSpeed, 'linear');
			}
			self.isActivated = false;
			$.event.trigger(self.options.customEventPrfx + ':popoverClosed', [self.popoverID]);
		});
...
/* /_ui/js/trans-path/classes/modal-popover.js */
/**
 * @author POP JS-ID [CN]
 * @version: 0.6.
 * @description: transforms inline (in-page) content into a modal popover window
 * @dependencies: jQuery 1.6+, POP.UI.ModalBase
 */
/*** start ModalPopover ***/
POP.UI.ModalPopover = function (trigger, options) {
	var self = this;

	// element references
    this.body = POP.CONFIG.body;
	this.elTrigger = trigger;
	this.targetID = this.elTrigger.attr('data-targetID');
	this.elTarget = $('#' + this.targetID);
	this.elPopover = false;

	// setup & properties
	this.options = $.extend({
		staticID: false,			// pass in ID str, else false
		popoverIDPrefix: 'popover',
		popoverClass: 'popover',
		includeCloseBtn: true,
		closeBtnClass: 'btn-closeX',
		closeBtnInnerHTML: '<span class="offscreen">Close(X)</span>',
		...
		backgroundClickClose: true,
		addHeaderAndFooter: false,
		customEventPrfx: 'POP:UI:ModalPopover'
	}, options || {});
	this._isActivated = false;
	this.popoverID = this.options.staticID || this.options.popoverIDPrefix + '-' + this.targetID;
	this.elTarget.hide();
...
POP.UI.ModalPopover.prototype = {
	bindEvents: function () {
		var self = this;

		this.body.bind(this.options.customEventPrfx + ':popoverOpened', function (event, popoverID) {
			if (popoverID === self.popoverID) {
				self._isActivated = self.popoverModalBase.isActivated;
			}
		});
		this.body.bind(this.options.customEventPrfx + ':popoverClosed', function (event, popoverID) {
			if (popoverID === self.popoverID) {
				self._isActivated = self.popoverModalBase.isActivated;
				self.elTrigger.focus();
			}
		});
		...
		this.elTrigger.bind('click', function (event) {
			...
			if (!self._isActivated) {
				self.popoverModalBase.openPopover();
			}
		});
	...
	initPopover: function () {
		...
		// init base class
		this.popoverModalBase = new POP.UI.ModalBase(this.popoverID, this.options);
		this.popoverModalBase.createWidgetMarkup();
		this.elPopover = this.popoverModalBase.elPopover;

		// inject content
		this.popoverContent = this.elTarget.html();
		this.elPopoverContent = this.elPopover.find('.' + this.options.popoverClass + '-content');
		this.elPopoverContent.html(this.popoverContent);
		this.popoverModalBase.setPosition();
	...
	setLoading: function () {
		this.elPopoverContent.addClass('loading');
		$('<div></div>').addClass('spinner').appendTo(this.elPopoverContent.parent());
	}
};
/*** end ModalPopover ***/

// start MultiModalPopoverController
POP.UI.MultiModalPopoverController = function (triggers, options) {
	this.elTriggers = triggers;
	this.options = options;
	this.arPopovers = [];
	this._len = this.elTriggers.length;
	this.processPopovers();
};
POP.UI.MultiModalPopoverController.prototype = {
	processPopovers: function () {
		var self = this,
			elTrigger = false;
		for (var i=0; i<this._len; i++) {
			elTrigger = $(this.elTriggers[i]);
			this.arPopovers[i] = new POP.UI.ModalPopover(elTrigger, this.options);
		}
	}
};
// end MultiModalPopoverController

/* /_ui/js/trans-path/classes/modal-image.js */
/**
 * @author POP JS-ID [CN]
 * @version: 0.6.
 * @description: takes a link to an image and injects into a modal popover window
 * @dependencies: jQuery 1.6+, POP.UI.ModalBase
 */
/*** start ModalImage ***/
POP.UI.ModalImage = function (trigger, options) {
	var self = this;

	// element references
    this.body = POP.CONFIG.body;
	this.elTrigger = trigger;
	this.imgURL = this.elTrigger.attr('data-imgUrl');
	this.elImage = false;
	this.elPopover = false;

	// setup & properties
	this.options = $.extend({
		popoverID: 'img-popover',
		popoverClass: 'img-popover',
		includeCloseBtn: true,
		closeBtnClass: 'btn-closeX',
		closeBtnInnerHTML: '<span class="offscreen">Close(X)</span>',
		activeClass: 'active',
		...
		backgroundClickClose: true,
		addHeaderAndFooter: false,
		customEventPrfx: 'POP:UI:ModalImage'
	}, options || {});
	this._isActivated = false;
	this.popoverID = this.options.popoverID;

	this.initPopover();

};

POP.UI.ModalImage.prototype = {
	bindEvents: function () {
		var self = this;
	...
	initPopover: function () {
		...
		// inject content
		this.elImage = $('<img src="' + this.imgURL + '" alt="" />');
		this.elPopoverContent = this.elPopover.find('.' + this.options.popoverClass + '-content');
		this.elPopoverContent.html(this.elImage);
		this.popoverModalBase.setPosition();
...
// start MultiModalImageController
POP.UI.MultiModalImageController = function (triggers, options) {
	this.elTriggers = triggers;
	this.options = $.extend({
		popoverID: 'img-popover'
	}, options || {});
...
POP.UI.MultiModalImageController.prototype = {
	processPopovers: function () {
		var self = this,
			elTrigger = false,
			refPopoverID = this.options.popoverID;
		for (var i=0; i<this._len; i++) {
			elTrigger = $(this.elTriggers[i]);
			this.options.popoverID = refPopoverID + '-' + i;
			this.arPopovers[i] = new POP.UI.ModalImage(elTrigger, this.options);
		}
	}
};
// end MultiModalImageController

/* /_ui/js/trans-path/classes/ajax-modal-popover.js */
/**
 * @author POP JS-ID [CN]
 * @version: 0.6.
 * @description: gets Ajax HTML content and injects into a modal popover window
 * @dependencies: jQuery 1.6+, POP.UI.ModalBase
 */
/*** start AjaxModalPopover ***/
POP.UI.AjaxModalPopover = function (trigger, options) {
	var self = this;

	// element references
    this.body = POP.CONFIG.body;
	this.elTrigger = trigger;
	this.modalSpinner = $('<div class="modalspinner"></div>');
	this.ajaxErrorMsg = '<div class="errormessage"><p>Sorry. Ajax request failed.</p></div>';

	// setup & properties
	this.options = $.extend({
		popoverID: 'ajaxpopover',
		popoverClass: 'popover',
		includeCloseBtn: true,
		closeBtnClass: 'btn-closeX',
		closeBtnInnerHTML: '<span class="offscreen">Close(X)</span>',
		...
		backgroundClickClose: true,
		addHeaderAndFooter: false,
		customEventPrfx: 'POP:UI:AjaxModalPopover',
	...
	this.ajaxContent = '';
	this.ajaxUrl = this.elTrigger.attr('data-ajaxUrl');
	this.splitAjaxUrl = (this.ajaxUrl) ? this.ajaxUrl.split('#') : [];
	this.ajaxUrlHref = this.splitAjaxUrl[0];
	this.targetHtmlID = this.splitAjaxUrl[1] || false;
	this.popoverID = this.options.popoverID;
...
POP.UI.AjaxModalPopover.prototype = {
	bindEvents: function () {
		var self = this;
	...
	initPopover: function () {
		...
		this.popoverModalBase = new POP.UI.ModalBase(this.popoverID, this.options);
		this.popoverModalBase.createWidgetMarkup();
		this.elPopoverContent = this.popoverModalBase.elPopover.find('.' + this.options.popoverClass + '-content');
	...
	initOpenOverlay: function () {
		...
		if (this.options.useModalOverlay) {
			this.popoverModalBase.modalOverlay.fadeIn(self.options.fadeInOutSpeed, 'linear', function () {
				self.popoverModalBase.modalOverlay.after(self.modalSpinner);
				self.modalSpinner.click();	//spinner gif gets 'stuck' and needs a click 
			});
		}
	...
	getAjaxContent: function () {
		...
		_injectContent = function () {
			self.elPopoverContent.html(self.ajaxContent);
			self.popoverModalBase.setPosition();
			setTimeout(function () {
				self.modalSpinner.hide().remove();
				self.popoverModalBase._openPopover();
			}, 100);	//add slight delay to show loading
		};

		this._ajaxInProgress = true;
		this.initOpenOverlay();
		$.ajax({
			url: self.ajaxUrlHref,
			dataType: 'html',
			success: function (results) {
				self._ajaxInProgress = false;
				self._ajaxLoaded = true;
				if (self.targetHtmlID) {
					self.ajaxContent = $(results).find('#' + self.targetHtmlID).html();
				} else {
					self.ajaxContent = results;
				}
				_injectContent();
				if (typeof self.options.onContentLoaded === 'function') {
					self.options.onContentLoaded(results);
				}
			},
			error: function (results) {
				...
				self._ajaxLoaded = true;
				_injectContent();
				if (typeof self.options.onContentError === 'function') {
					self.options.onContentError(results);
				}
			}
		});
...
/*** end AjaxModalPopover ***/

// start MultiAjaxModalPopoverController
POP.UI.MultiAjaxModalPopoverController = function (triggers, options) {
	this.elTriggers = triggers;
	this.options = $.extend({
		popoverID: 'ajaxpopover'
	}, options || {});
...
POP.UI.MultiAjaxModalPopoverController.prototype = {
	processPopovers: function () {
		var self = this,
			elTrigger = false,
			refPopoverID = this.options.popoverID;
		for (var i=0; i<this._len; i++) {
			elTrigger = $(this.elTriggers[i]);
			this.options.popoverID = refPopoverID + '-' + i;
			this.arPopovers[i] = new POP.UI.AjaxModalPopover(elTrigger, this.options);
		}
	}
};
// end MultiAjaxModalPopoverController

/* /_ui/js/trans-path/classes/accordion.js */
/**
 * @author POP JS-ID [CN]
 * @version: 0.6.
 * @description: standard accordion
 * @dependencies: jQuery 1.6+, POP.UTILS
 */
/*** start Accordion ***/
POP.UI.Accordion = function (container, options) {
	var self = this;
	...
	this.options = $.extend({
		initialIndex: 0,
		tabSelector: 'a.acc-tab',
		panelSelector: 'div.acc-panel',
		activeClass: 'active',
		updateHash: false,
		setEqualHeight: false,
		selfClosing: false,
		animDuration: 400,
		customEventPrfx: 'POP:UI:Accordion'
	}, options || {});

	// element references
	this.elTabs = this.elContainer.find(this.options.tabSelector);
	this.elPanels = this.elContainer.find(this.options.panelSelector);

	// setup & properties
	this._isInitialized = false;
	this._isAnimating = false;
	this._len = this.elTabs.size();
	if (this.options.initialIndex >= this._len) {this.options.initialIndex = 0;}
	this.currentIndex = this.options.initialIndex;
	this.nextIndex = 0;

	// check url hash to override currentIndex
	this.urlHashCheck = false;
	this.urlHash = window.location.hash.replace('#','') || false;
	if (this.urlHash) {
		for (var i=0; i<this._len; i++) {
			if (this.elPanels[i].id === this.urlHash) {
				this.currentIndex = i;
				this.urlHashCheck = true;
				break;
			}
		}
	}
...
POP.UI.Accordion.prototype = {
    bindEvents: function () {
        var self = this;

        POP.CONFIG.window.bind('hashchange', function (event) {
            self.__onHashChange();
        });

        if (!this.elTabs.eq(0).attr('data-set')) {
            this.elTabs.bind('click', function (event) {
                event.preventDefault();
                if (!self._isAnimating) {
                    self.__clickTab(event);
                }
            });

            this.elTabs.eq(0).attr('data-set', true);
        }
    },
    initDisplay: function () {
        var self = this,
			...
			currentTab = $(this.elTabs[index]),
			currentPanel = $(this.elPanels[index]);

        // set equal heights
        if (this.options.setEqualHeight) {
            this.panelHeightAdjuster = new POP.UTILS.HeightAdjuster(this.elPanels);
        }
        ...
        currentPanel.attr({ 'aria-expanded': 'true', 'aria-hidden': 'false' }).addClass(this.options.activeClass).show();

        if (this.urlHashCheck) {
        ...
        $.event.trigger(this.options.customEventPrfx + ':isInitialized', [index]);
    ...
    animateSelfOpen: function () {
        var self = this,
			tab = $(this.elTabs[this.nextIndex]),
			panel = $(this.elPanels[this.nextIndex]);
    ...
    animateAccordion: function () {
        var self = this,
			nextTab = $(this.elTabs[this.nextIndex]),
			nextPanel = $(this.elPanels[this.nextIndex]),
			currentTab = $(this.elTabs[this.currentIndex]),
			currentPanel = $(this.elPanels[this.currentIndex]),
...
/* /_ui/js/trans-path/classes/dialog.js */
(function () {

'use strict';


POP.UI.Dialog = function (trigger, buttons, options) {
	var self = this;
	...
	this.trigger.on('click', function (e) {
		...
		// build dialog
		var overlay = $('<div></div>').addClass('dialog-overlay').appendTo($(document.body));
		var dialog = $('<div></div>').addClass(self.options.className).
			attr('aria-hidden', false).attr('aria-expanded', true).
			html(self.options.content);

		self.elDialog = dialog;
		self.elOverlay = overlay;
		// make buttons
		var btnList = $('<ul></ul>').addClass(self.options.className + '-buttons').appendTo(dialog);
		var button;
		var buttonItem;
		var buttonClick;
		for (var i = 0, ii = self.buttons.length; i < ii; i++) {
			button = $('<a></a>').prop({
				'href': '#',
				'tabindex': '0'
			}).html(self.buttons[i].text);
			if (typeof self.buttons[i].onClick === 'function') {
				buttonClick = $.proxy(self.buttons[i].onClick, self);
			} else {
				buttonClick = $.proxy(self.close, self);
			}
			console.log(buttonClick);
			if (Object.prototype.toString.call(self.buttons[i].classList) === '[object Array]') {
				for (var j = 0, jj = self.buttons[i].classList.length; j < jj; j++) {
					button.addClass(self.buttons[i].classList[j]);
				}
			}
			buttonItem = $('<li></li>').html(button).appendTo(btnList);
			button.on('click', buttonClick);
		}

		$(document.body).append(dialog);

		var width = dialog.get()[0].offsetWidth;
		...
		dialog.find('li a').first().focus();

		dialog.find('li a').last().on('blur', function (e) {
			e.preventDefault();
			e.stopPropagation();
			dialog.find('li a').first().focus();
		});
...
POP.UI.Dialog.prototype = {
	close: function () {
		var self = this;
		this.elOverlay.fadeOut();
		this.elDialog.fadeOut(function () {
			self.elDialog.attr('aria-hidden', true).attr('aria-expanded', false).remove();		
...
/* /_ui/js/trans-path/classes/infinite-carousel.js */
/**
 * @author POP JS-ID [CN]
 * @version: 0.6.
 * @description: infinitely looping carousel
 * @dependencies: jQuery 1.6+
 */
/*** start InfiniteCarousel ***/
POP.UI.InfiniteCarousel = function (container, options) {
	var self = this;
	...
	this.options = $.extend({
		...
		itemsPerGroup: 5,
		visibleItems: 5,
		prevLinkSelector: 'a.carousel-lnk-prev',
		nextLinkSelector: 'a.carousel-lnk-next',
		...
		isInfinite: true,
		animationSpeed: 600,
		customEventPrfx: 'POP:UI:InfiniteCarousel'
	}, options || {});
	...
	this._userInteraction = false;						// bool: has the user interacted with widget?
	this._len = this.elItems.size();					// num: computed length of items
	if (this.options.initialIndex >= this._len) {this.options.initialIndex = 0;}			// reset initial index if too high
	this._autoRotateCounter = this._len * this.options.maxAutoRotations;					// num: counter for looping thru items one full rotation
	if (this._len <= this.options.visibleItems) {								// if not enough items...
		this.options.autoRotate = false;										// disable autoRotate
		this.options.isInfinite = false;										// disable isInfinite
		this.elPrevLink.addClass(this.options.disabledLinkClass);				// disable elPrevLink
		this.elNextLink.addClass(this.options.disabledLinkClass);				// disable elNextLink
	}				
	this.currentIndex = this.options.initialIndex;								// currentIndex = first item
	this.previousIndex = this.currentIndex;										// declare previousIndex
	this.firstPos = 0;															// init track position
	this.lastPos = 0;															// init track position
	this.scrollAmt = (this.elItems.outerWidth() * this.options.itemsPerGroup) * -1;			// amt to scroll by = items width
...
POP.UI.InfiniteCarousel.prototype = {
	bindEvents: function () {
		var self = this;
	...
	initDisplay: function () {
		...
		this.elPrevLink.attr({'role': 'button', 'aria-controls': this.elContainer.attr('id')});
		this.elNextLink.attr({'role': 'button', 'aria-controls': this.elContainer.attr('id')});
	...
	handleRotation: function () {
		...
		this.slideCarousel();
		this._autoRotateCounter--;
		if (this._autoRotateCounter == 0) {
    ...
    adjustPosition: function () {
		...
		if (this._userInteraction) {
			this.elItems[this.currentIndex].focus();
		}
		$.event.trigger(this.options.customEventPrfx + ':carouselUpdated');
...
/* /_ui/js/trans-path/classes/media-carousel.js */
/**
 * @author POP JS-ID [CN]
 * @version: 0.5.
 * @description: image gallery carousel controller
 * @dependencies: jQuery 1.5+, POP.UTILS, POP.UI.InfiniteCarousel, youtube player api
 */
/*** start MediaCarousel ***/
POP.UI.MediaCarousel = function (navCarousel, container, options) {
	var self = this;

	// defaults
	this.navCarousel = navCarousel;
	this.elContainer = container;
	this.options = $.extend({
		panelsSelector: 'div.panel',
		overlaySelector: 'div.overlay',
		spinnerSelector: 'div.spinner',
		activeClass: 'active',					// str: set css class for active elelents
		useSpinnerTransition: true,				// bool: transition the panels directly or use a spinner to indicate change?
		autoPlay: false,						// bool: auto play first youtube video
		autoRotate: true,						// bool: auto rotate thru all items
		...
		useFadeInOutEffect: true,				// bool: default (true), override (false) = 'show/hide'
		animationSpeed: 400,					// num: duration (in ms) of animation effect (useFadeInOutEffect must also be true)
		imgLazyLoad: false,						// bool: set true to convert img src from data-src
		deactivateOnHover: true,				// bool: stop auto rotation on mouse hover
		customEventPrfx: 'POP:UI:MediaCarousel'
	}, options || {});

	// element references
	this.elPanels = this.elContainer.find(this.options.panelsSelector);
	this.elOverlay = this.elContainer.find(this.options.overlaySelector);
	this.elSpinner = this.elContainer.find(this.options.spinnerSelector);
	this.elNavItemParents = this.navCarousel.elItems;
	this.elNavItems = this.elNavItemParents.find('a');
	this.elPrevLink = this.navCarousel.elPrevLink;
	this.elNextLink = this.navCarousel.elNextLink;

	// setup & properties
	this._isInitialized = false;						// is widget initialized yet?
	this._isAnimating = false;							// is widget currently animating?
	this._userInteraction = false;						// has the user interacted with widget?
	this._len = this.elPanels.size();
	this._controlsNavCarousel = true;
	if (this._len <= 1) {
		// disable carousel
		this._controlsNavCarousel = false;
		this.deactivateAutoRotate();
		this.navCarousel.elContainer.remove();
	}
	if (this._len <= this.navCarousel.options.visibleItems) {
		// disable carousel rotation control if not enough items
		this._controlsNavCarousel = false;
	}
	this._autoRotateCounter = this._len * this.options.maxAutoRotations;		// num: counter for looping thru items one full rotation
	this.initialIndex = this.navCarousel.options.initialIndex;
	this.currentIndex = this.initialIndex;				// currentIndex = first item
	this.prevIndex = -1;								// declare previousIndex

	// youtube video player properties
	this.elYTPlayers = $('#media-carousel-panels iframe');
	this.playersLen = this.elYTPlayers.length;
	this.ytPlayers = [];
	...
	if (this.urlHash) {
		for (var i = 0; i < this._len; i++) {
			if ($(this.elPanels[i]).attr('id') === this.urlHash) {
				//TODO: implement hash override
				//this.currentIndex = i;
				this.urlHashCheck = true;
				break;
			}
...
POP.UI.MediaCarousel.prototype = {
	bindEvents: function () {
		var self = this;

		window.focus();
		window.onfocusout = window.onblur = function () {
			self.options.autoRotate = false;
			clearInterval(self.setAutoRotation);
			window.onfocusout = window.onblur = '';
		};

		POP.CONFIG.body.bind('POP:UI:NavCarousel:carouselUpdated', function (event) {
			//console.log('POP:UI:NavCarousel:carouselUpdated');
		});

		POP.CONFIG.body.bind('POP:YouTubePlayerAPI:Ready', function (event) {
			//console.log('POP:YouTubePlayerAPI:Ready');
			self.initVideoPlayers();
		});

		POP.CONFIG.body.bind('POP:FirstYouTubePlayer:Ready', function (event, obYTPlayer) {
			//console.log('POP:FirstYouTubePlayer:Ready');
			obYTPlayer.target.playVideo();		
		});

		if (this.options.deactivateOnHover) {
			this.elContainer.bind({
				mouseenter: function(event) {
					self._userInteraction = true;
					self.deactivateAutoRotate();
				},
		...
		this.elNavItems.bind('click', function (event) {
			event.preventDefault();
			if (!self._isAnimating) {
				self._userInteraction = true;
				self.deactivateAutoRotate();
				self.__clickCarouselItem(event);
			}
		});

		POP.CONFIG.window.load(function () {
			// lazy-load images
			if (self.options.imgLazyLoad) {
				self.lazyloadImages();
			}
		});
	...
	initDisplay: function () {
		...
		// add aria attributes to container, tabs, and panels
		this.elContainer.attr({'role': 'tablist', 'aria-multiselectable': 'false'});
		this.elPanels.attr({'role': 'tabpanel', 'tabindex': '-1', 'aria-expanded': 'false', 'aria-hidden': 'true'}).hide();
		$(this.elPanels[index]).attr({'aria-expanded': 'true', 'aria-hidden': 'false'}).show();

		// add active class to all three versions of thumb to cover all bases instead of checking which one is currently visible
		$(this.elNavItemParents[index]).addClass(this.options.activeClass);
		$(this.elNavItemParents[index + this._len]).addClass(this.options.activeClass);
		$(this.elNavItemParents[index + this._len + this._len]).addClass(this.options.activeClass);
	...
	/*
		pseudo image lazy-load: simply waits until page load is complete
		before loading img src from data-src
	*/
    lazyloadImages: function () {
		...
		this.elPanels.each(function(i){
			img = $(this).find('img');
			imgDS = img.attr('data-src');
			img.attr('src', imgDS);
    ...
    __clickCarouselItem: function (event) {
		var self = this,
			el = $(event.currentTarget),
			index = this.elNavItems.index(el),
			imgName;

		if (el.parent().hasClass(this.options.activeClass)) {
			$(this.elPanels[this.currentIndex]).focus();
		} else {

			imgName = el.find('img').attr('src').split('/');
			...
			} else if (index < this._len + this._len) {
				this.currentIndex = index - this._len;
			} else {
				this.currentIndex = index - (this._len + this._len);
			}
    ...
    updateNavItems: function () {
		...
		this.elNavItemParents.removeClass(this.options.activeClass);
		$(this.elNavItemParents[index]).addClass(this.options.activeClass);

		if (this._controlsNavCarousel) {
			$(this.elNavItemParents[this.currentIndex + this._len]).addClass(this.options.activeClass);
			$(this.elNavItemParents[this.currentIndex + this._len + this._len]).addClass(this.options.activeClass);
			if (this.options.autoRotate) {
				this.navCarousel.currentIndex++;
				this.navCarousel.slideCarousel();
			}
		}

    },
    transitionMediaPanels: function () {
		var self = this,
			prevPanel = $(this.elPanels[this.prevIndex]),
			nextPanel = $(this.elPanels[this.currentIndex]),
			transDelay = this.options.useSpinnerTransition ? this.options.animationSpeed : 0;

		$.event.trigger(this.options.customEventPrfx + ':panelSwitched');
		this.stopVideoPlayers();
		this._isAnimating = true;
		if (this.options.useSpinnerTransition) {
			this.elOverlay.show();
			this.elSpinner.show();
		}
		prevPanel.css({'z-index': 1});
		nextPanel.css({'z-index': 2, 'display': 'block', 'opacity': 0.0});
		nextPanel.delay(transDelay).animate({
			opacity: 1.0
		}, self.options.animationSpeed, 'linear', function(){
			if (self.options.useSpinnerTransition) {
				self.elOverlay.hide();
				self.elSpinner.hide();
			}
			prevPanel.attr({'aria-expanded': 'false', 'aria-hidden': 'true'}).removeClass(self.options.activeClass).hide();
			nextPanel.attr({'aria-expanded': 'true', 'aria-hidden': 'false'}).addClass(self.options.activeClass);
	...
	*/
	initVideoPlayers: function () {
		...
		if (POP.CONFIG.isIE7) {return;}

		for (var i=0; i<this.playersLen; i++) {
			playerId = this.elYTPlayers[i].id;
			if (self.options.autoPlay && i === 0) {
				this.ytPlayers[i] = new YT.Player(playerId, {
					events: {'onReady': onFirstYouTubePlayerReady}
				});
			} else {
				this.ytPlayers[i] = new YT.Player(playerId);
			}
	...
	stopVideoPlayers: function () {
		...
		for (var i=0; i<this.playersLen; i++) {
			if (typeof this.ytPlayers[i].getPlayerState == 'function') {
				playerState = this.ytPlayers[i].getPlayerState();
				if (playerState == 1) {
					this.ytPlayers[i].pauseVideo();
				}
			}
		}
    }
...
function onFirstYouTubePlayerReady(obYTPlayer) {
	$.event.trigger('POP:FirstYouTubePlayer:Ready', [obYTPlayer]);
}

function onYouTubePlayerAPIReady() {
	$.event.trigger('POP:YouTubePlayerAPI:Ready');
}

/* /_ui/js/trans-path/classes/revealer.js */
/**
 * @author POP JS-ID [CN]
 * @version: 0.6.
 * @description: standard revealer
 * @dependencies: jQuery 1.6+
 */
/*** start Revealer ***/
POP.UI.Revealer = function (container, options) {
	var self = this;
	...
	this.options = $.extend({
		initRevealed: false,
		triggerSelector: 'a.revealer-trigger',
		targetSelector: 'div.revealer-target',
		activeClass: 'active',
		useSlideEffect: true,
		animDuration: 400,
		includeCloseLink: false,
		closeLinkClass: 'lnk-close',
		customEventPrfx: 'POP:UI:Revealer'
	}, options || {});

	// element references
	this.elTrigger = this.elContainer.find(this.options.triggerSelector).first();
	this.elTarget = this.elContainer.find(this.options.targetSelector).first();
	this.elCloseLink = this.options.includeCloseLink ? $('<a class="' + this.options.closeLinkClass + '" href="#close" title="close window"><span>Close</span></a>') : false;

	// setup & properties
	if ((this.elTrigger.length === 0) || (this.elTarget.length === 0)) {return;}
	this.targetID = this.elTarget.attr('id');
	this.triggerText = this.elTrigger.html();
	this.activeTriggerText = this.elTrigger.attr('data-activeText') || false;
	this._isAnimating = false;
	this._isRevealed = (this.options.initRevealed || this.elTarget.attr('data-initRevealed') == 'true') ? true : false;

	// check urlHash
	this.hashOverride = false;
	this.urlHash = window.location.hash.replace('#','') || false;
	if (this.urlHash && this.urlHash === this.elTarget.attr('id')) {
		this._isRevealed = true;
		this.hashOverride = true;
	}
...
POP.UI.Revealer.prototype = {
	bindEvents: function () {
		...
		if (this.options.includeCloseLink) {
			this.elCloseLink.bind('click', function (event) {
				if (self._isRevealed) {
					event.preventDefault();
	...
	initDisplay: function () {
		...
		if (this.options.includeCloseLink) {
			this.elTarget.append(this.elCloseLink);
		}
		if (this._isRevealed) {
		...
		if (this.hashOverride) {this.elTarget.focus();}
	},
	...
	revealContent: function () {
		...
		if (this.options.useSlideEffect) {
			...
			this.elTarget.show().focus();
			this._isRevealed = true;
			this.elContainer.addClass(this.options.activeClass);
			$.event.trigger(this.options.customEventPrfx + ':contentRevealed', [this.elContainer]);
		}
	...
	collapseContent: function () {
		...
		if (this.activeTriggerText) {
			this.elTrigger.html(this.triggerText);
		}
		...
		if (this.options.useSlideEffect) {
			...
			this.elContainer.removeClass(this.options.activeClass);
			$.event.trigger(this.options.customEventPrfx + ':contentCollapsed', [this.elContainer]);
		}
...
POP.UI.MultiRevealerController = function (containers, options) {
	this.elContainers = containers;
	this.options = options;
	this.arRevealers = [];
	this._len = this.elContainers.length;
	this.processRevealers();
};
POP.UI.MultiRevealerController.prototype = {
	processRevealers: function () {
		var self = this,
			elContainer = false;
		for (var i=0; i<this._len; i++) {
			elContainer = $(this.elContainers[i]);
			this.arRevealers[i] = new POP.UI.Revealer(elContainer, this.options);
		}
	}
};
// end MultiRevealerController

/* /_ui/js/trans-path/classes/toggler.js */
/**
 * @author POP JS-ID [CN]
 * @version: 0.6.
 * @description: toggles show/hide between short/long content area
 * @dependencies: jQuery 1.6+
 */
/*** start Toggler ***/
POP.UI.Toggler = function (container, options) {
	var self = this;
	...
	this.options = $.extend({
		initToggled: false,
		triggerSelector: 'a.toggler-trigger',
		targetSelector: 'div.toggler-target',
		altTargetSelector: 'div.toggler-alt-target',
		activeClass: 'active',
		useAppearEffect: true,
		animDuration: 400,
		customEventPrfx: 'POP:UI:Toggler'
	}, options || {});

	// element references
	this.elTrigger = this.elContainer.find(this.options.triggerSelector).first();
	this.elTarget = this.elContainer.find(this.options.targetSelector).first();
	this.elAltTarget = this.elContainer.find(this.options.altTargetSelector).first();

	// setup & properties
	this.targetID = this.elTarget.attr('id');
	this.altTargetID = this.elAltTarget.attr('id');
	this.triggerText = this.elTrigger.html();
	this.activeTriggerText = this.elTrigger.attr('data-activeText') || false;
	this._isAnimating = false;
...
POP.UI.Toggler.prototype = {
	...
	initDisplay: function () {
		...
		if (this._isToggled) {
			...
			ariaExpanded = 'true';
			ariaHidden = 'false';
			targetID = this.altTargetID;
			this.elContainer.addClass(this.options.activeClass);
		}

		// add aria attributes
		this.elContainer.attr({'role': 'tablist', 'aria-multiselectable': 'false'});
		this.elTrigger.attr({'role': 'tab', 'aria-selected': ariaSelected, 'aria-controls': targetID});
		this.elTarget.attr({'role': 'tabpanel', 'tabindex': '-1', 'aria-expanded': ariaExpanded, 'aria-hidden': ariaHidden});
	...
	collapseContent: function () {
		...
		this.elTrigger.attr({'aria-selected': 'false'});
...
POP.UI.MultiTogglerController = function (containers, options) {
	this.elContainers = containers;
	this.options = options;
	this.arTogglers = [];
	this._len = this.elContainers.length;
	this.processTogglers();
};
POP.UI.MultiTogglerController.prototype = {
	processTogglers: function () {
		var self = this,
			elContainer = false;
		for (var i=0; i<this._len; i++) {
			elContainer = $(this.elContainers[i]);
			this.arTogglers[i] = new POP.UI.Toggler(elContainer, this.options);
		}
	}
};
// end MultiTogglerController

/* /_ui/js/trans-path/classes/tooltip.js */
/**
 * @author POP JS-ID [CN]
 * @version: 0.6.
 ...
 */
/*** start Tooltip ***/
POP.UI.Tooltip = function (triggers, options) {
	var self = this;
	...
	this.options = $.extend({
		...
		hoverDelay: 200,
		addHeaderAndFooter: false,
		customEventPrfx: 'POP:UI:Tooltip'
	}, options || {});
	...
	this._isActivated = false;
	this.overTimeout = false;				// mouseenter timeout
	this.outTimeout = false;				// mouseleave timeout
...
POP.UI.Tooltip.prototype = {
	...
	initTooltip: function () {
		...
		this.tooltipHTML = '<div id="' + this.tooltipID + '" class="' + this.options.tooltipClass + ' ' + this.tooltipPositionClass + ' ' + '" role="tooltip" tabindex="-1" aria-expanded="false" aria-hidden="true">';
		if (this.options.addHeaderAndFooter) {
			this.tooltipHTML += '	<div class="' + this.options.tooltipClass + '-header"></div>'
							 +  '	<div class="' + this.options.tooltipClass + '-content"></div>'
							 +  '	<div class="' + this.options.tooltipClass + '-footer"></div>';
		} else {
			this.tooltipHTML += '	<div class="' + this.options.tooltipClass + '-content"></div>';
		}
	...
	__triggerMouseenter: function (event) {
		var self = this;
...
// start MultiTooltipController
POP.UI.MultiTooltipController = function (triggers, options) {
	this.elTriggers = triggers;
	this.options = options;
	this.arTooltips = [];
	this._len = this.elTriggers.length;
...
POP.UI.MultiTooltipController.prototype = {
	processTooltips: function () {
		var self = this,
			elTrigger = false;
		for (var i=0; i<this._len; i++) {
			elTrigger = $(this.elTriggers[i]);
			this.arTooltips[i] = new POP.UI.Tooltip(elTrigger, this.options);
		}
	}
};
// end MultiTooltipController

/* /_ui/js/trans-path/utils.js */
...
POP.UTILS.fixIE = {
	addClasses: function (selectorClassPair) {
		var selector, elements, cssClass, el;
		for (selector in selectorClassPair) {
			elements = $(selector), cssClass = selectorClassPair[selector];
...
**/
POP.UTILS.SubmitForm = function (formContainer, submitButton) {
	var self = this;
	this.elFormContainer = formContainer;
	this.elSubmitButton = submitButton;
	this.elFormContainer.find('input,select').keydown(function (event) {
		if (event.keyCode == '13') {
			event.preventDefault();
			self.elSubmitButton.click();
		}
	});
/*
// old way
	this.elFormContainer.on('keyup', function (event) {
...
/**
*	Replace submit buttons with links (for Cufon)
**/
POP.UTILS.ReplaceButtons = (function () {
	this.elContainer = false;
	this.elButtons = false;
	function __replace(elContainer){
		this.elContainer = elContainer || POP.CONFIG.body;
		this.elButtons = this.elContainer.find('input[type=submit]').not('#search-events input, .search-field input, .HiddenButton, .hide-if-js');
		this.elButtons.each(function (i, val) {
			var elButton = $(this),
				text = elButton.val(),
				btnClass = elButton.attr('class'),
				elLink,
				disabled;
			elButton.attr({'tabindex': '-1'});
			disabled = ( elButton.hasClass('disabled') ) ? 'disabled' : '';
			elLink = $('<a href="#" class="' + btnClass + ' ' + disabled + '">' + text + '</a>');
			elLink.insertAfter(elButton);
			elLink.bind('click', function (e) {
				e.preventDefault();
				if (!elLink.hasClass('disabled')) {
					elButton.click();
				}
			});
		});
		this.elButtons.addClass('offscreen').attr({'tabindex': '-1'});
	}
	return {
		replace: function (elContainer) {
			__replace(elContainer);
		}
	}
}());
/*** /ReplaceButtons ***/
...
*	Print Page
**/
POP.UTILS.PrintPage = function (links) {
	var self = this;
	this.links = links;
...
**/
POP.UTILS.HeightAdjuster = function (items, options) {
	var self = this;
	this.items = items;
	this._len = items.size();
...
POP.UTILS.HeightAdjuster.prototype = {
...
**/
POP.UTILS.TextareaMaxlength = function ($textarea) {
	var self = this;
...
POP.UTILS.TextareaMaxlength.prototype = {
	enforce: function () {
		var self = this;
		if (this.textarea.val().length >= this.maxlength) {
			this.textarea.val(self.textarea.val().substring(0, self.maxlength));
		}
	}
};
/*** /TextareaMaxlength ***/
...
POP.UTILS.PopupWindow = function (triggers, options) {
	var self = this;
	this.triggers = triggers;
	this._len = triggers.length;
	...
	this.triggers.bind('click', function (event) {
		event.preventDefault();
		self.__click(event);
	});
};
POP.UTILS.PopupWindow.prototype = {
	__click: function (event) {
		var el = event.currentTarget;
		this.openPopup(el);
	},
	openPopup: function (el) {
		var index = this.triggers.index(el);
		var url = $(this.triggers[index]).attr('href');
...
*	ClosePopup
**/
POP.UTILS.ClosePopup = function (trigger) {
	var self = this;
	this.trigger = trigger;
...
**/
POP.UTILS.AjaxLoader = function (target, options) {
...
POP.UTILS.AjaxLoader.prototype = {
...
/**
*	Select dropdown activates textbox
**/
POP.UTILS.SelectboxActivatesTextbox = function (selectbox, textbox, options) {
	var self = this;
	this.selectbox = selectbox;
	this.textbox = textbox;
	this.label = $('label[for=' + this.textbox.attr('id') + ']');
	this.options = $.extend({
		optionTriggerVal: '0',
...
POP.UTILS.SelectboxActivatesTextbox.prototype = {
	__change: function () {
		if (this.selectbox.val() === this.options.optionTriggerVal) {
			this.activate();
		} else {
			this.deactivate();
		}
	...
	activate: function () {
		this.label.removeClass(this.options.disabledClass);
		this.textbox.removeClass(this.options.disabledClass).removeAttr('disabled').focus();
	},
	deactivate: function () {
		this.label.addClass(this.options.disabledClass);
		this.textbox.addClass(this.options.disabledClass).attr('disabled','disabled').val('');
	}
};
// end SelectboxActivatesTextbox

/* /_ui/js/trans-path/init.js */

/**
*	Add a class name to the html tag as a global flag that the user has javascript enabled
**/
...
SOH.Main = {
	init: function () {
		var self = this;
		//console.log('SFS.Main.init');

		POP.CONFIG.window = $(window);
		POP.CONFIG.body = $('body');
		POP.CONFIG.aspnetForm = $('#aspnetForm');
		POP.UI.modalOverlay = $('#modaloverlay');

		// *** element references *** //
		this.elPrintLnks = $('a.lnk-print');
		this.elColumns = $('#main-col, #sub-col');

		// *** init widgets *** //
		POP.UTILS.ReplaceButtons.replace();
		this._initGlobalWidgets();
		if (POP.CONFIG.isIE) {
			this._initIEFixes();
		}

		// Overriding iOS :hover clever handling ...
		// Jira Ticket: SS-22482
		if ( 'ontouchstart' in document.documentElement ) {
			document.documentElement.className += " touch";
		}
	},

	_initGlobalWidgets: function () {
		var self = this;

		this.printPage = new POP.UTILS.PrintPage(this.elPrintLnks);
	...
	_initIEFixes: function () {
		...
		// IE6 support removed
		if (POP.CONFIG.isIE6) {
			POP.UTILS.fixIE.addClasses({
				//'input[type=text]': 'input-type-text'
			});
			POP.UTILS.fixIE.addHoverClasses({
				//'div.selector-example-1 li' : 'hover',
				//'div.selector-example-2 img' : 'hover'
			});
		}
		*/
		// exempt IE9
		if (!POP.CONFIG.isIE9) {
			POP.UTILS.fixIE.addClasses({
				// IE examples
				//'table.data-grid tbody tr:nth-child(odd)' : 'nth-child-odd',
				//'table.data-grid tbody tr:nth-child(even)' : 'nth-child-even',
				//'div.photo-library-grid ul li:nth-child(3n+1)' : 'nth-child-first'
			});
		}

		if (POP.CONFIG.isIE7) {
			// this.columnHeightAdjuster = new POP.UTILS.HeightAdjuster(this.elColumns);
			$('#main-col').css({height: 'auto'});
		}
